...
#' The function allows to compute the effect size in terms of the naive HR,the unweighted average hazard ratio,
#' the geometric average hazard ratio, the average hazard ratio, the ratio of restricted mean survival time and the median ratio.
#' 
...
#'
#' \describe{
#'   \item{\code{nHR}}{naive HR}
#'   \item{\code{uAHR}}{unweighted Average Hazard Ratio}
#'   \item{\code{gAHR}}{geometric Average Hazard Ratio}
#'   \item{\code{AHR}}{unweighted Average Hazard Ratio}
...
#' 
#' All returned values should be interpreted in relative terms (treated to control). Parameters nHR, uAHR, gAHR, AHR represent the risk reduction for the new treatment, while RMST_ratio and Median_ratio represent the gain in terms of time to event.
#' nHR is the arithmetic mean for the HR of the components.
#'
#'
...
effectsize_tte <- function(p0_e1, p0_e2, HR_e1, HR_e2, beta_e1=1, beta_e2=1, case, copula = 'Frank', rho=0.3, rho_type='Spearman', subdivisions=1000, plot_HR=FALSE){
  ...
  if(plot_HR){
    ...
    gg1 <- ggplot(dd, aes(x=t,y=HR)) + geom_line(color='blue',size=2) + 
      ylim(ymin,ymax) + 
      ggtitle('HR of the composite endpoint') + xlab('Time') + ylab('HR*')
    print(gg1)
  }
  
  return(list('naive HR' = round(nHR,4),
              'mean HR' = round(mHR,4),
              'sAHR' = round(sAHR_0,4),
              'gAHR' = round(gAHR,4),
              'AHR' = round(AHR,4),

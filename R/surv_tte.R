...
surv_tte <- function(p0_e1, p0_e2, HR_e1, HR_e2, beta_e1=1, beta_e2=1, case,
  ...
  if(plot_print | plot_save){
    ...
    ## Composite endpoint
    gg3 <- ggplot() +
      geom_line(mapping = aes(x = t, y = Sstar0, color = "Control"), linetype = 'longdash', size = 1.3) +
      geom_line(mapping = aes(x = t, y = Sstar1, color = "Treated"), linetype = 'longdash', size = 1.3) +
      scale_y_continuous(limits = c(0, 1), minor_breaks = NULL, expand = c(0, 0)) +
      scale_x_continuous(limits = c(0, 1),
                         breaks = pretty(0:1 * f_time) / f_time,
                         labels = pretty(0:1 * f_time),
                         expand = c(0, 0.01)) +
      xlab('Time') +
      ylab('Survival CE') +

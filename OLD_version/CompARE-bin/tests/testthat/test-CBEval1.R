...
test_that("CBEval1 works", {
  ...
  ##BLOC A - Validacions per a la funció prob_ce
  ...
  #A
  expect_equal(prob_ce_1, prob_ce_2)
  expect_equal(prob_ce_3, prob_ce_2)
  expect_equal(prob_ce_1, prob_ce_3)
  expect_false(prob_ce_1, prob_ce_4)
  ...
  #C
  expect_that(length(prob_ce_1),is_identical_to(length(prob_ce_2)),is_identical_to(length(prob_ce_3)),is_identical_to(length(prob_ce_4)))
  ...
  #BLOC B - Validacions de les funcions que s'utilitzen per calcular correlacions

  Lower_corr_1 <- lower_corr(0.2,0.3)
  Lower_corr_2 <- lower_corr(0.5,0.5)   #individus amb 50% de probabilitat
  Upper_corr_1 <- upper_corr(0.6,0.4)
  Upper_corr_2 <- upper_corr(0.2,0.2)  #individus amb 20% de probabilitat
  Upper_corr_3 <- upper_corr(0.2,0.3)

  #A
  expect_true(Upper_corr_2 == 1)
  expect_true(Lower_corr_2 == -1)
  expect_false(Lower_corr_1 == Upper_corr_3)
  ...
  #C
  expect_that(length(Lower_corr_1),is_identical_to(length(Lower_corr_1)))
  expect_that(length(Upper_corr_1),is_identical_to(length(Upper_corr_2)),is_identical_to(length(Upper_corr_3)))
  ...
  # BLOC C - Validacio de la funció que calcula els efectes de CBE
  ...
  #BLOC D - Validació de la grandària mostral
  ...
  limits <- function(parm, inf, sup){
    if(parm == "alpha"){
      for (i in seq(inf,sup, 0.01)) {
        expect_true(sample_size_ce(0.2,0.3,0.1,0.2,"diff",0.4, i,2,"unpooled Variance"))
      }
    }else if(parm == "beta"){
      for (i in seq(inf,sup, 0.01)) {
        expect_true(sample_size_ce(0.2,0.3,0.1,0.2,"diff",0.4, 0.05,i,"unpooled Variance"))
      }
    }
  }
